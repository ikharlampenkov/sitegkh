...
<a href="?page={$page}&action=add">ДОБАВИТЬ ДОМ</a> / <a href="?page={$page}&action=add_street">ДОБАВИТЬ УЛИЦУ</a> / <a href="#" style="color:#aaaaaa">ИМПОРТИРОВАТЬ ДАННЫЕ ИЗ CVS-ФАЙЛА</a>
</td>
</table>
<table cellpadding="5" border="0" style="font-size:14px;">
    <tr>
...
{foreach from=$house_list item=house}
	...
	<td class="pem">{$house.number}{if !empty($house.subnumber)}{$house.subnumber}{/if}</td>
        ...
        <td class="pom"><a href="?page={$page}&action={if $house.is_street==1}edit_street{else}edit{/if}&id={$house.id}">редактировать</a></td>
        <td class="pom"><a href="?page={$page}&action={if $house.is_street==1}{else}del_street{/if}&id={$house.id}">удалить</a></td>
    </tr>
{/foreach}
</table>
{/if}
