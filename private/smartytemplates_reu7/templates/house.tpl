...
{if $action == "view"}
    ...
    {if !empty($house.file_repair_plan)}
        <a href="{$siteurl}temp_files/{$house.file_repair_plan}" target="_blank">План работ по содержанию и ремонту</a>
        <br/>
        <br/>
    {/if}
    ...
    <table width="100%" cellpadding="5" cellspacing="2">
        <tr valign="top">
            ...
            <td class="pem" style="font-size:14px" width="33%">
                <span><a href="javascript:showHouse({$smarty.foreach._house_list.index})">{$street.street.street}</a></span>

                <div id="house_{$smarty.foreach._house_list.index}" style="display: none;">
                    {if isset($street.street.file_repair_plan)}<a href="{$siteurl}temp_files/{$street.street.file_repair_plan}" target="_blank">План работ по содержанию и ремонту</a><br/>{/if}
                    {if isset($street.street.file_costs_income)}<a href="{$siteurl}temp_files/{$street.street.file_costs_income}" target="_blank">Доходы и расходы</a><br/>{/if}
            ...
            {if $smarty.foreach._house_list.iteration%3==0}</tr>
        <tr valign="top">{/if}
            {/foreach}
        </tr>
    </table>
