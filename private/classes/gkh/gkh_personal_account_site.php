...
class gkh_personal_account_site extends gkh
{
    ...
    public function getPAByLS($ls)
    {
        try {
            $sql = 'SELECT * FROM personal_account WHERE ls="' . $ls . '"';
            $result = $this->_db->query($sql, simo_db::QUERY_MOD_ASSOC);
            if (!empty($result[0])) {
                return $result[0];
            } else {
    ...
    public function checkPA($ls)
    {
        try {
            $sql = 'SELECT COUNT(id) as cnt FROM personal_account WHERE ls="' . $ls . '"';
            $result = $this->_db->query($sql, simo_db::QUERY_MOD_ASSOC);
            if (!empty($result[0]['cnt']) && $result[0]['cnt'] > 0) {
                return true;
            } else {
                return false;
            }
    ...
    public function addPA($data)
    {
        try {
            ...
            $user_data = array('login' => 'user_ntenant_' . $o_temp_user->getNextId(), 'password' => $data['password'], 'role' => 'ntenant');
            $o_temp_user->addUser($user_data);

            $temp_id = $this->_db->query('SELECT LAST_INSERT_ID()');

            $sql = 'INSERT INTO personal_account(user_id, house_id, apartment, fio, password, ls)
                    VALUES(' . $temp_id[0][0] . ', 0,
                           0, "' . $data['fio'] . '", "' . $data['password'] . '", "' . $data['ls'] . '")';
            $this->_db->query($sql);
        } catch (Exception $e) {
            simo_exception::registrMsg($e, $this->_debug);
        }
    ...
    public function updatePA($id, $data)
    {
        try {
            ...
            $sql = 'UPDATE personal_account
                       SET house_id=0, fio="' . $data['fio'] . '", apartment=0,
                           password="' . $data['password'] . '", ls="' . $data['ls'] . '"
                     WHERE id=' . (int)$id;
            $this->_db->query($sql);


            $sql = 'SELECT user_id FROM personal_account WHERE id="' . $id . '"';
            ...
            if (isset($result[0][0])) {
                ...
                $temp_user->editUser($user['id'], $user);
    ...
    public function filterPA($filterList)
    {
        try {
            $paList = $this->getAllPA();

            foreach ($paList as $pa) {
                if (!in_array($pa['ls'], $filterList)) {
                    $sql = 'DELETE FROM user WHERE id=' . (int)$pa['user_id'];
                    $this->_db->query($sql);
    ...
    static function getUserByPA($personal_account)
    {
        try {
            ...
            $sql = 'SELECT user_id FROM personal_account WHERE ls="' . $personal_account . '"';
            $result = $db->query($sql);
