        ...
        try {
            ...
            if (isset($result[0])) {
                ...
                foreach ($result as $res) {
                    ...
                    $retArray[$i]['street']['street'] = $res['street'];
                    if ($res['is_street'] == 0) {
                    ...
                    $j++;
                       ...
                       $retArray[$i]['street']['file_repair_plan'] = $res['file_repair_plan']; 
                       $retArray[$i]['street']['file_costs_income'] = $res['file_costs_income']; 
                    }
    ...
    public function addHouse($data) {
        try {
            ...
            if (strpos($data['number'], ',') !== false) {
                ...
                $sql = 'INSERT INTO house(street, number, subnumber, area, is_street)
                    VALUES("' . $data['street'] . '", ' . $data['number'] . ', "' . $data['subnumber'] . '", ' . str_replace(',', '.', $data['area']) . ', ' . $data['is_street'] . ')';
                $this->_db->query($sql);
            }
        } catch (Exception $e) {
            simo_exception::registrMsg($e, $this->_debug);
        ...
        try {
            ...
            $sql = 'UPDATE house
                    SET street="' . $data['street'] . '", number=' . $data['number'] . ',
                        subnumber="' . $data['subnumber'] . '", area=' . str_replace(',', '.', $data['area']) . ', is_street=' . $data['is_street'] . ' 
                    WHERE id=' . (int) $id;
