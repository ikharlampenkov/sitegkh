...
class gkh_document extends gkh
{
    ...
    public function addDocument($data)
    {
        try {
            ...
            $sql = 'INSERT INTO document(parrent_id, title, short_text, is_folder)
                    VALUES(' . $data['parrent_id'] . ', "' . $data['title'] . '", "' . $data['shorttext'] . '", ' . $data['is_folder'] . ')';
            $this->_db->query($sql);
        ...
        try {
            ...
            $sql = 'UPDATE document
                    SET parrent_id=' . $data['parrent_id'] . ', title="' . $data['title'] . '",
                        short_text="' . $data['shorttext'] . '", is_folder=' . $data['is_folder'] . '
                    WHERE id=' . (int)$id;
            $this->_db->query($sql);
    ...
    protected function _uploadFile($id, $field)
    {
        ...
        if (isset($_FILES['data'])) {
            ...
            if (!empty($_FILES['data']['name'][$field])) {
                ...
                //echo $__cfg['file.upload.dir'] . $temp_file_name;
                $result = copy($_FILES['data']['tmp_name'][$field], $__cfg['file.upload.dir'] . $temp_file_name);
    ...
    public function deleteFile($id, $field)
    {
        ...
        try {
            ...
            simo_functions::_delFile($__cfg['file.upload.dir'] . $temp[$field]);
