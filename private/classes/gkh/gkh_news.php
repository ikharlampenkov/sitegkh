...
class gkh_news extends gkh
{
        ...
        try {
            ...
            if (isset($result[0])) {
                foreach ($result as &$res) {
                    $res['title'] = stripcslashes(stripslashes($res['title']));
                    $res['short_text'] = stripcslashes(stripslashes($res['short_text']));
                    $res['full_text'] = stripcslashes(stripslashes($res['full_text']));
                }
                return $result;
            } else
                return false;
    ...
    public function getNews($id)
    {
        try {
            ...
            $result = $this->_db->query($sql, simo_db::QUERY_MOD_ASSOC);
            if (isset($result[0])) {
                $result[0]['title'] = stripcslashes(stripslashes($result[0]['title']));
                $result[0]['short_text'] = stripcslashes(stripslashes($result[0]['short_text']));
