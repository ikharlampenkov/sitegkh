...
class gkh_tech_support_post_site extends gkh_tech_support_post
{
    ...
    public function askQuestion($data, $category, $pa = 0)
    {
        try {
            ...
            if (!empty($data['question'])) {
                ...
                if ($pa == 0) {
                    $pa = $this->_personal_account;
                }

                $sql = 'INSERT INTO tech_support_ticket(personal_account_id, title, date, category, ticket_status_id) 
                        VALUES(' . $pa . ', "' . $title . '", "' . $date . '", "' . $category . '", ' . gkh_tech_support_post::NEW_TICKET_STATUS . ')';
                $this->_db->query($sql);
    ...
    {
        try {
            $sql = 'DELETE FROM tech_support_post WHERE ticket_id=' . (int)$id;
            $this->_db->query($sql);

            $sql = 'DELETE FROM tech_support_ticket WHERE id=' . (int)$id;
            $this->_db->query($sql);
