<div style="color: #838383; font-size: 21px; border-bottom: 2px solid #9c9a9a; padding-bottom: 10px;">Контентные страницы</div>
                ...
                <td>
                    {if isset($conpage.file_list) && $conpage.file_list !== false}
                        {foreach from=$conpage.file_list item=file name=_file}
                            <a href="{$siteurl}temp_files/{$file}" target="_blank">Файл {$smarty.foreach._file.iteration}</a>
                            &nbsp;
                            <a href="?page={$page}&action=del_pic&id={$conpage.id}&fname={$file}">удалить</a>
                            <br/>
                        {/foreach}
                    {/if}
