<div class="page_title">Вопросы и ответы</div>
...
<form action="?page={$page}&action={$action}&id={if $action=="edit"}{$faq.id}{/if}" method="post" enctype="multipart/form-data">
    <table>
        ...
        <tr class="pem">
            <td>Ответ</td>
            <td>{$ckeditor} {* <textarea name="data[answer]">{$faq.answer}</textarea>*}</td>
        </tr>
        <tr class="pem">
            ...
            <td><select name="data[parrent_id]">
                    ...
                    <option value="{$folder.id}" {if isset($faq.parrent_id) && $faq.parrent_id==$folder.id}selected="selected"{/if}>{$folder.question}</option>
                    {/foreach}
                </select></td>
        </tr>
        <tr class="pem">
...
<form action="?page={$page}" method="post">
    <table width="100%">
        <tr>
            <td width="150" class="ttovar">E-mail для писем</td>
            <td class="ttovar"><input name="info[email]" value="{$email}"/></td>
        </tr>
    </table>
    <input id="save" name="save" type="submit" value="Сохранить"/>
</form><br/><br/>
...
{foreach from=$faq_list item=faq}
	...
	<td class="pem">{$faq.answer|strip_tags:false|truncate:200}</td>
